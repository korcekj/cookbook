...
const IngredientItem: FC<IngredientItemProps> = ({
  ...
  return (
    <li
      key={title}
      className='flex items-center space-x-2 p-2 border border-gray-100 rounded'
    >
      <input
        type='checkbox'
        className='w-4 h-4 text-gray-600 bg-gray-100 rounded border-gray-300 focus:ring-gray-600 focus:ring-2'
      />
      <div className='text-base font-light text-gray-900 space-x-1 overflow-x-auto'>
        <span className='font-medium'>{title}</span>
        <span>
          ({value % 1 !== 0 ? toFraction(value) : value} {unit})
        </span>
      </div>
    </li>
  );
